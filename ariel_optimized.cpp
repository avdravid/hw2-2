...
// 2D processor grid
int proc_grid_dims[2];
int proc_grid_coords[2];
MPI_Comm cart_comm;
...
struct particle_with_cell {
    particle_t p;
    int cell_idx; // 1D index into flat_grid
};

// Flat grid structure
struct flat_cell_t {
    int start; // Start index in particle_indices
    int count; // Number of particles
};

// Global grid and particle data
...
void build_grid() {
    ...
    // Fill indices with a second pass
...
void setup_2d_decomposition(int num_procs) {
    ...
    int coords[2];
    if (proc_grid_coords[0] > 0) { coords[0] = proc_grid_coords[0] - 1; coords[1] = proc_grid_coords[1]; MPI_Cart_rank(cart_comm, coords, &neighbors[0]); }
    if (proc_grid_coords[0] < proc_grid_dims[0] - 1) { coords[0] = proc_grid_coords[0] + 1; coords[1] = proc_grid_coords[1]; MPI_Cart_rank(cart_comm, coords, &neighbors[1]); }
    if (proc_grid_coords[1] > 0) { coords[0] = proc_grid_coords[0]; coords[1] = proc_grid_coords[1] - 1; MPI_Cart_rank(cart_comm, coords, &neighbors[2]); }
    if (proc_grid_coords[1] < proc_grid_dims[1] - 1) { coords[0] = proc_grid_coords[0]; coords[1] = proc_grid_coords[1] + 1; MPI_Cart_rank(cart_comm, coords, &neighbors[3]); }
    if (proc_grid_coords[0] > 0 && proc_grid_coords[1] > 0) { coords[0] = proc_grid_coords[0] - 1; coords[1] = proc_grid_coords[1] - 1; MPI_Cart_rank(cart_comm, coords, &neighbors[4]); }
    if (proc_grid_coords[0] < proc_grid_dims[0] - 1 && proc_grid_coords[1] > 0) { coords[0] = proc_grid_coords[0] + 1; coords[1] = proc_grid_coords[1] - 1; MPI_Cart_rank(cart_comm, coords, &neighbors[5]); }
    if (proc_grid_coords[0] > 0 && proc_grid_coords[1] < proc_grid_dims[1] - 1) { coords[0] = proc_grid_coords[0] - 1; coords[1] = proc_grid_coords[1] + 1; MPI_Cart_rank(cart_comm, coords, &neighbors[6]); }
    if (proc_grid_coords[0] < proc_grid_dims[0] - 1 && proc_grid_coords[1] < proc_grid_dims[1] - 1) { coords[0] = proc_grid_coords[0] + 1; coords[1] = proc_grid_coords[1] + 1; MPI_Cart_rank(cart_comm, coords, &neighbors[7]); }
}
...
void exchange_ghost_particles() {
    ...
    int req_count = 0;
    int send_counts[8], recv_counts[8];
    for (int i = 0; i < 8; i++) {
    ...
    for (size_t i = 0; i < local_particles.size(); i++) {
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                ...
                const flat_cell_t& cell = flat_grid[n_idx];
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    int local_count = temp_local.size();
